...
IF li_ret = 1 Then
	IF ldws_status = Newmodified! Then
		...
		adw_persondetail.ImportRowFromJson(ls_json, li_row)
...
IF li_ret = 1 Then
	...
	adw_master.ImportJson(ls_jsonaddress)
	adw_detail.ImportJson(ls_jsonphone)
	adw_cust.ImportJson(ls_customer)
...
inv_restclient.of_execute_func(ls_url, lstr_am, ljs_pak)
IF li_ret = -1 Then Return -1
